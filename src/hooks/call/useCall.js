...
export function useCall(payload, socket, is_caller = false, token) {
    ...
    const endCall = async () => {
        ...
        const { success, data, error, message } = await ACall.Update(payload?.service_id, { field: "status:complete", data: {} }, token);
        if (success) {
    ...
    const RejectCall = async () => {
        ...
        peer.current = null;
        localStream.current?.getTracks().forEach(t => t.stop());
        const { success, data, error, message } = await ACall.Update(payload?.service_id, { field: "status:cancelled", data: {} }, token);
        if (success) {
            stopTimer();
            EndCallCleanup();
    ...
    const PlayRingtone = () => {
        ...
        ringtoneRef.current.currentTime = 0; // optional
